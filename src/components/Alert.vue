<script setup>
const emit = defineEmits(['closeAlert'])

const props = defineProps({
    message: {
        type: String,
        default: 'A simple alert with text and a right icon',
    },
    class: {
        type: String,
        default: 'text-red-700 bg-red-100',
    }
})

const setAlertStatus = () => {
    emit('closeAlert', false)
}

</script>

<template>
  <div class="max-w-full lg:w-2/4 sm:w-1/2 mx-auto mt-20">
    <div
      class="relative py-3 pl-4 pr-10 leading-normal rounded-lg"
      :class="props.class"
      role="alert"
    >
      <p v-text="props.message"></p>
      <span class="absolute inset-y-0 right-0 flex items-center mr-4" @click="setAlertStatus">
        <svg class="w-4 h-4 fill-current" role="button" viewBox="0 0 20 20">
          <path
            d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
            clip-rule="evenodd"
            fill-rule="evenodd"
          ></path>
        </svg>
      </span>
    </div>
  </div>
</template>
